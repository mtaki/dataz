...
$html.="<tr align='left' style=\"background-color:#D7DEEE;\"><th>Invoice Id</th><th>Number</th><th>Date</th><th>Description</th><th>Amount</th><th>Amount Due</th></tr>";
$las=Invoice::model()->findAll(array('condition'=>"operator_id=".$model->id,'order'=>'t.id desc limit 5'));
foreach ($las as $la){
	$html.="<tr align='left' class='odd'><td>".$la->id."</td><td>$la->num</td><td>".date('d-m-Y',strtotime($la->day))."</td><td>$la->description</td><td>$la->amount ".$la->currency->name."</td><td>$la->amount_due ".$la->currency->name."</td></tr>";
...
<?php $this->beginWidget('system.web.widgets.CClipWidget', array('id'=>'Licences')); ?>
	<?php 
		$html="<table width='600' class='detail-view'>";
$html.="<tr align='left' style=\"background-color:#D7DEEE;\"><th>Id</th><th>Number</th><th>Type</th><th>Issue date</th><th>Duration</th></tr>";
...
foreach ($las as $la){
	$html.="<tr align='left' class='odd'><td>$la->id</td><td>$la->num</td><td>".$la->licenceCategory->name."</td><td>$la->issue_date</td><td>$la->duration</td></tr>";
