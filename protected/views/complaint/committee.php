<script language="javascript">
	...
	function appeal(){
		var a = confirm('Record Appeal ?');
		if (a == true){
			document.getElementById('workflowAction').value='Appeal';
			document.getElementById('workflowform').submit();
		}
		else
			return;
...
<?php
	...
	if (!empty($_REQUEST['action']) && $_REQUEST['action']=='addHistory'){
		...
		$log=new Log;
						...
						$log->action=$_REQUEST['action_type'];
						...
						$log->team=$_REQUEST['team']; 
						$log->stage='TCRA Committee';
						$log->save(false);
	}
	elseif (!empty($_REQUEST['action']) && $_REQUEST['action']=='Resolved'){
		$model->stage='TCRA Committee';
		$model->status='Closed';
		$model->save(false);
	}
	elseif (!empty($_REQUEST['action']) && $_REQUEST['action']=='Appeal'){
		$model->stage='Tribunal';
		$model->status='Appeal';
		$model->save(false);
	}
...
echo $mainTable;
echo $historyTable;
if ($model->stage=='TCRA Committee' && $model->status!='Closed'){?>
	<table width='100%' border='0'>
		<tr  style="background-color:#D7DEEE;"><td width='70%'>ADD HISTORY</td><td>WORK FLOW</td></tr>
		<tr>
			<td>
				...
				<table class='detail-view'>
						...
						<tr  class='odd'><td valign='top'>DECISION/COMMENT:</td><td><textarea rows='3' cols='25' name='decision'></textarea></td></tr>
						<tr  class='odd'>
					...
					<option value='ruling'>Ruling</option>
						...
						<tr  class='odd'><td valign='top'>TEAM:</td><td valign='top'><textarea rows='3' cols='25' name='team'></textarea></td></tr>
						
						
						<tr  class='odd'><td valign='top'>&nbsp;</td><td><input type='button' value='Add History' onclick='addHistory()'/></td></tr>				
				</table>
			...
			<td valign='top'>
				...
				<table class='detail-view'>
						<tr class='even'><td><input type='button' value='Resolved' onclick='resolve()'/></td></tr>
						<tr class='even'><td><input type='button' value='Appeal' onclick='appeal()'/></td></tr>
				</table>
				<input type='hidden' name='entityid' value='$id'/>
				<input type='hidden' name='action' value='' id='workflowAction'/>
				<input type='hidden' name='page' value='complaints/committee.php'/>
				</form>
			</td>
		</tr>
	</table>	
